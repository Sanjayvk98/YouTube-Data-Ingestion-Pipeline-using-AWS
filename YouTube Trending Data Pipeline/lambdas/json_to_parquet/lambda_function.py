...
SNS_TOPIC = os.environ.get("SNS_ALERT_TOPIC_ARN") or os.environ.get("SNS_ALERT_TOPIC_NAME", "")
...
def resolve_sns_topic_arn(topic_value: str) -> str:
    """Return a full SNS TopicArn from either an ARN or a plain topic name."""
    if not topic_value:
        return ""

    if topic_value.startswith("arn:aws:sns:"):
        return topic_value
    ...
    try:
        ...
        for page in paginator.paginate():
            for topic in page.get("Topics", []):
                ...
                if topic_arn.rsplit(":", 1)[-1] == topic_value:
                    return topic_arn
    except Exception as exc:
        logger.warning(f"Unable to resolve SNS topic name '{topic_value}' to ARN: {exc}")
...
def send_alert(subject: str, message: str):
    if not SNS_TOPIC:
        logger.info("SNS alert topic is not configured; skipping alert.")
        return

    topic_arn = resolve_sns_topic_arn(SNS_TOPIC)
    if not topic_arn:
        logger.warning(
            f"SNS topic '{SNS_TOPIC}' is not a valid ARN or resolvable name; skipping alert. "
            "Set SNS_ALERT_TOPIC_ARN to the full ARN value."
        )
        return
